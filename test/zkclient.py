...
def watchmethod(func):
    def decorated(handle, atype, state, path):
        ...
        return func(event,path)
    return decorated

class ZKClient(object):
    def __init__(self, servers, timeout=DEFAULT_TIMEOUT, connect_watcher=None):
        self.timeout = timeout
        self.connect_watcher = connect_watcher
        ...
        start = time.time()
        self.handle = zookeeper.init(servers, self.connection_init_watcher, timeout)
